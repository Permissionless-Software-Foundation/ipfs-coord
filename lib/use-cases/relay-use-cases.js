    ...
    try {
      ...
      if (circuitRelayInfo) {
        if (circuitRelayInfo.ip4) {
          const ip4Multiaddr = `/ip4/${circuitRelayInfo.ip4}/tcp/${circuitRelayInfo.tcpPort}/p2p/${peerData.from}`
          peerData.data.ipfsMultiaddrs.push(ip4Multiaddr)
        }
  ...
  async measureRelays (thisNode) {
    try {
      ...
      for (let i = 0; i < thisNode.relayData.length; i++) {
        ...
        // Get the peer data.
        let peerData = thisNode.peerData.filter(
          x => x.data.ipfsId === thisRelay.ipfsId
        )
        peerData = peerData[0]
  ...
  removeDuplicates (thisNode) {
    try {
      ...
      const endingRelays = startingRelays.filter(
        (relay, index, self) =>
          index === self.findIndex(t => t.ipfsId === relay.ipfsId)
      )
