...
class RelayUseCases {
  ...
  async connectToCRs (thisNode) {
    ...
    knownRelays = this._sortRelays(knownRelays)
    ...
    try {
      ...
      for (let i = 0; i < relayData.length; i++) {
        ...
        if (thisRelay.isBootstrap) {
          ...
          thisRelay.latencyScore = this._average(thisRelay.metrics.aboutLatency)
      ...
      const sortedList = relayData.sort((a, b) =>
        a.latencyScore > b.latencyScore ? 1 : -1
      )
      console.log('sortedList: ', sortedList)

      return sortedList
    } catch (err) {
      console.error('Error in _sortRelays()')
      throw err
    }
  }
  ...
  async addRelay (ipfsId, thisNode) {
    try {
      ...
      const alreadyInList = thisNode.relayData.filter(x =>
        x.ipfsId.includes(ipfsId)
      )
      // Exit if this peer is already in the list of Relays.
      if (alreadyInList.length) return true

      console.log('New Circuit Relay Peer Found!')
      ...
      for (let i = 0; i < peerData.data.ipfsMultiaddrs.length; i++) {
        ...
        if (connectSuccess) {
          ...
          if (!isIp4 && !isIp6 && !isDns) {
            console.log(`Rejecting Relay multiaddr ${multiaddr}`)
  ...
  async measureRelays (thisNode) {
    try {
      ...
      for (let i = 0; i < thisNode.relayData.length; i++) {
        ...
        const testResult = await this.adapters.about.queryAbout(
          thisRelay.ipfsId,
          thisNode
        )
