...
class RelayUseCases {
  ...
  async addRelay (ipfsId, thisNode) {
    try {
      ...
      // TODO: I may need to add this peer to a blacklist, so that this node does
      // not keep trying to connect to the bad relay.
      if (!connectSuccess) {
      ...
      thisNode.relayData.push(newRelayObj)
      console.log(`New Circuit Relay added: ${thisAddr}`)
  ...
  async validateRelay (ipfsId) {
    try {
      ...
      console.error('Error in validateRelay()')
      throw err
    }
  }
}
