  ...
  async connectToCRs () {
    for (let i = 0; i < this.state.relays.length; i++) {
      ...
      const connectedPeer = connectedPeers.filter(
        peerObj => thisRelay.multiaddr.match(peerObj.peer)
      )

      // If this node is already connected to the peer, the skip this node.
      ...
      try {
        ...
        console.log(`Successfully connected to Circuit Relay node ${thisRelay.multiaddr}`)
      } catch (err) {
        /* exit quietly */
        console.warn(`Error trying to connect to circuit-relay ${thisRelay.multiaddr}: ${err.message}`)
      }
    }
