...
class IpfsAdapter {
  constructor (localConfig = {}) {
    ...
    this.state = {
      ...
      isCircuitRelay: localConfig.isCircuitRelay
        ? localConfig.isCircuitRelay
        : false,

      // A string of text. Usually an IPFS hash or a web URL.
      apiInfo: localConfig.apiInfo,

      // Schema.org and JSON-LD information to overwrite the default schema.
      announceJsonLd: localConfig.announceJsonLd,
  ...
  async connectToPeer (ipfsId) {
    try {
      await this.ipfs.swarm.connect(ipfsId)

      console.log(`Successfully connected to peer node ${ipfsId}`)
      ...
      // )
      console.warn(`Error trying to connect to peer node ${ipfsId}`)
  ...
  async start () {
    try {
      ...
      console.error('Error in ipfs-adapters.js/start()')
      throw err
    }
  }
  ...
  async initAsyncLibs () {
    try {
      ...
      const schemaConfig = {
        ...
        announceJsonLd: this.state.announceJsonLd
      }
