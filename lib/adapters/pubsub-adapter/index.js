...
class PubsubAdapter {
  constructor (localConfig = {}) {
    ...
    if (!this.eventEmitter) {
      throw new Error(
        'An instance of Event Emitter must be passed when instantiating the Pubsub Adapter libary'
      )
    }
  ...
  async handleNewMessage (msgObj, handler, thisNode) {
    try {
      ...
      // being measured.
      const isAbout = this.captureMetrics(msgObj.data.payload, msgObj.from, thisNode)

      // Pass the JSON RPC data to the private log to be handled by the app
      // consuming this library.
      if (!isAbout) { this.privateLog(msgObj.data.payload, msgObj.from) }
    } catch (err) {
  ...
  async captureMetrics (decryptedStr, from, thisNode) {
    try {
      ...
      if (data.id.includes('metrics')) {
        ...
        await this.messaging.sendMsg(from, jsonResponse, thisNode)
