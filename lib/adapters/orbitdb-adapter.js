  ...
  async handleReplicationEvent (orbitdbId) {
    try {
      ...
      if (!alreadyProcessed) {
        ...
        const isMetricData = _this.captureMetrics(decryptedStr)
        // console.log("isMetricData: ", isMetricData);

        // If it's not metric data, then pass the data onto the private-log,
        // to be processed by whatever app is consuming this library.
        if (!isMetricData) _this.privateLog(decryptedStr, result[0].from)
      }
