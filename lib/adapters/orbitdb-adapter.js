...
class OrbitDBAdapter {
  ...
  async handleReplicationEvent (orbitdbId) {
    try {
      ...
      const newData = this.db.iterator({ limit: 1 }).collect()
      // console.log(`newData: ${JSON.stringify(newData, null, 2)}`)

      const hash = newData[0].hash

      const alreadyProcessed = this._checkIfAlreadyProcessed(hash)
      ...
      if (!alreadyProcessed) {
        ...
        const decryptedStr = await this.encryption.decryptMsg(result[0].data)
        // console.log('decryptedStr: ', decryptedStr)

        this.privateLog(decryptedStr, result[0].from)
      }
    } catch (err) {
