...
class OrbitDBAdapter {
  constructor (localConfig = {}) {
    ...
    // console.log(`OrbitDB contructor node type: ${this.nodeType}`)
  ...
  async createRcvDb (selfData) {
    ...
    try {
      ...
      this.db = await this.orbitdb.eventlog(dbName.toString(), options)
      ...
      console.error('Error in orbitdb.js/createRcvDb()')
      throw err
    }
  }
    ...
    try {
      ...
      if (!thisNode.orbitList.includes(peerId)) {
        ...
        if (dbData[0].orbitdbId !== orbitdbId) {
          ...
          // Disconnect and delete the old database.
          const oldDb = dbData[0].db
          await oldDb.close()
          await oldDb.drop()
          _this.log.statusLog(
            2,
            `Closed connection to old OrbitDB ${dbData[0].orbitdbId}`
          )
          _this.log.statusLog(
            2,
            `Opening connection to new OrbitDB ${orbitdbId}`
          )
          ...
          thisNode.orbitData[dbIndex] = newDbData
        }
      }
    } catch (err) {
      console.error('Error in orbitdb.js/connectToPeerDb(): ', err)
      throw err
    }
  }
}
