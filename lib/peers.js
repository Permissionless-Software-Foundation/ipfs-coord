...
class Peer {
  ...
  // Deprecated. Moved to this-node-use-cases.js
  ...
  async addPeer (announceObj) {
    try {
      ...
      await _this.orbitdb.connectToPeerDb(
        announceObj.from,
        announceObj.data.orbitdb
      )
    } catch (err) {
    ...
    try {
      ...
      for (const peer in this.state.peers) {
        ...
        const connectedPeer = connectedPeers.filter(
          peerObj => peerObj.peer === thisPeer.ipfsId
        )

        // If this node is already connected to the peer, the skip this peers.
        ...
        for (let i = 0; i < relays.length; i++) {
          ...
          try {
            ...
            /* exit quietly, and try the next Circuit Relay */
