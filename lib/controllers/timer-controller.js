...
class TimerControllers {
  ...
  async monitorBandwidth (thisNode, useCases) {
    try {
      ...
      for await (const stats of ipfs.stats.bw()) {
        // console.log(stats)
        this.adapters.log.statusLog(2, 'Bandwidth stats: ')
        this.adapters.log.statusLog(2, stats)
      }

      const bitswap = await ipfs.stats.bitswap()
      // console.log('bitswap: ', bitswap)
      this.adapters.log.statusLog(2, bitswap)
  ...
  async managePeers (thisNode, useCases) {
    ...
    try {
      ...
      // console.error('Error in timer-controller.js/manageAnnouncement(): ', err)
      this.adapters.log.statusLog(1, 'Renewed connections to all subnet peers.')
